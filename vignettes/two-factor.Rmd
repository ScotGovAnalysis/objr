...
Some of the required changes can only be made using the API (and not in the [Objective Connect browser interface](https://secure.objectiveconnect.co.uk/login)). If either your Workgroup Admin or Workspace Owner is not an R user, they can use the API via it's [interactive documentation](https://secure.objectiveconnect.co.uk/publicapi/1/swagger-ui/index.html) instead of using objr.

Links to relevant sections of the interactive documentation will be provided for each step below.

[More detailed guidance to use the interactive documentation](#interactive) is available in the Getting more help section.
...
### Using the Interactive Documentation {#interactive}

Some of the required changes can only be made using the API. If either your Workgroup Admin or Workspace Owner is not an R user, they can use the API via it’s [interactive documentation](https://secure.objectiveconnect.co.uk/publicapi/1/swagger-ui/index.html) instead of using objr.

Links to relevant sections of the interactive documentation are provided for each step above.

Before using the interactivity, set up API authentication by clicking the green 'Authorise' button (near the top of the page on the right hand side).
...
#|    'Authorize' button on the right hand side of the view.
knitr::include_graphics("authorize.png", dpi = "retina")
```

In the pop-up window, enter your username (usually your email address) and password for Objective Connect, and click the 'Authorize' button.
...
#| fig-align: "center"
#| fig-alt: >
#|    A screenshot of the authorisation pop-up window with input boxes to enter
#|    your username and password. An 'Authorize' button is underneath.
knitr::include_graphics("auth-pop-up.png", dpi = "retina")
```

To use the API within each section, click the 'Try it out' button. You will then be able to enter values for required parameters. 
...
#|    out' button on the right hand side of the section for the bypass two step
#|    endpoint. 
knitr::include_graphics("try-it-out.png", dpi = "retina")
```

Once you have supplied the required information, click the blue 'Execute' button to send the API request. Below this you will be able to see the API response, and whether it has been successful.
